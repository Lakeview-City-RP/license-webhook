...
def create_license_image(
    ...
    # ========================
    # CARD BASE + ROUNDED MASK
    # ========================
    card = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    base = Image.new("RGBA", (W, H), (255, 255, 255, 0))

    mask = Image.new("L", (W, H), 0)
    ImageDraw.Draw(mask).rounded_rectangle((0, 0, W, H), radius=120, fill=255)
    ...
    # ========================
    # HEADER (CURVED EXACTLY TO LICENSE TOP)
    # ========================
    HEADER_H = 115
    ...
    # Use *top slice* of card mask so header has identical top curve
    header_mask = mask.crop((0, 0, W, HEADER_H))
    header.putalpha(header_mask)

    # Header is at the VERY TOP of the license
    ...
    draw.text(((W - tw) / 2, 26), title, fill="white", font=title_font)
