...
# ---------- LICENSE IMAGE ----------
def create_license_image(username, avatar_bytes, roleplay_name, age, address, eye_color, height,
                         issued, expires, lic_num):
    """Creates a Lakeview City DMV Driver License"""
    W, H = 850, 520
    ...
    # Centered banner with Roblox name
    banner = f"CITY OF LAKEVIEW OFFICIAL USE ONLY • Roblox: {username}"
    ...
    if avatar_bytes:
        try:
            ...
            draw.rounded_rectangle((50, 170, 230, 350),
                                   radius=40, outline=(150, 160, 180), width=3)
    ...
    def draw_label_value(label, value, y):
        draw.text((xL, y), f"{label}", fill=text_color, font=font_bold)
        lw = draw.textlength(label, font=font_bold)
        draw.text((xL + lw + 10, y), value or "N/A", fill=text_color, font=font_text)

    draw_label_value("Name:", roleplay_name or username, yL + spacing)
    draw_label_value("Age:", age, yL + spacing*2)
    draw_label_value("Address:", address, yL + spacing*3)

    # Physical info (right)
    xR, yR = 560, 160
    draw.text((xR, yR), "PHYSICAL INFO", fill=accent, font=font_bold)
    draw_label_value("Eye Color:", eye_color, yR + spacing)
    draw_label_value("Height:", height, yR + spacing*2)

    # DMV Notes
    notes_top = 370
    draw.rounded_rectangle((30, notes_top, W - 30, H - 30),
                           radius=25, outline=(160, 170, 190),
                           width=2, fill=(235, 238, 250))
    draw.text((50, notes_top + 8), "DMV NOTES", fill=accent, font=font_bold)
    draw.text((50, notes_top + 45),
    ...
    sdraw.text((25, 60), "Lakeview City\nDMV Certified",
               font=font_small, fill=(220, 180, 80, 180), align="center")
...
def license_endpoint():
    try:
        ...
        img_bytes = create_license_image(username, avatar_bytes, roleplay_name,
        ...
        async def send_license():
            ...
            if channel:
                ...
                await channel.send(
                    ...
                    embed=embed,
                    file=new_file(img_bytes, filename))
            if discord_id:
                try:
